  ...
  var productTemplate = `
    ...
    <div class="product-details">
        <div class="product-description">
            ...
            <h4 class="brand-name" >${clickedProduct[urlId].brand}</h4>
            <h3 class="section-heading">Price:Rs <span id="price">${clickedProduct[urlId].price}</span></h3>
...
const cartShow=document.querySelector(".cart-count span")
console.log(cart);
...
function cartCount(){
    let proCount=localStorage.getItem("itemTotal")
    proCount=parseInt(proCount);
   
    if(proCount==0){
        localStorage.getItem("itemTotal",proCount + 1);
        cartShow.textContent=proCount + 1;
        console.log(cartShow)
    }else{
        localStorage.setItem("itemTotal",1)
        // cartShow.textContent=proCount = 1;
    }
